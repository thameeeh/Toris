...
namespace OutlandHaven.UIToolkit
{
    public class ForgeSubView : UIView
    {
        ...
        {
            // Create a dummy slot with quantity 1 for visual representation in the Forge
            InventorySlot visualSlot = new InventorySlot();
            visualSlot.SetItem(new ItemInstance(slot.HeldItem.BaseItem, slot.HeldItem.CurrentLevel), 1);

            if (_currentSlot1Data == null)
            {
                _currentSlot1Data = visualSlot;
                _slot1View?.Update(visualSlot);
            }
            else if (_currentSlot2Data == null)
            {
                _currentSlot2Data = visualSlot;
                _slot2View?.Update(visualSlot);
            }
