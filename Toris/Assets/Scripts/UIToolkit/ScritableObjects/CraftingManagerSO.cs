...
namespace OutlandHaven.UIToolkit
{
    ...
    public class CraftingManagerSO : ScriptableObject
    {
        ...
        private void HandleRequestForge(InventorySlot slot1, InventorySlot slot2)
        {
            ...
            // Determine which is base and which is material to get correct quantities
            int item1Quantity = 1;
            int item2Quantity = 1;

            if (recipe.BaseItemRequirement == item1)
            {
                var mat = recipe.MaterialRequirements.Find(m => m.Material == item2);
                item2Quantity = mat.Quantity > 0 ? mat.Quantity : 1;
            }
            else
            {
                var mat = recipe.MaterialRequirements.Find(m => m.Material == item1);
                item1Quantity = mat.Quantity > 0 ? mat.Quantity : 1;
            }

            // Attempt to remove inputs from player inventory
            bool removedSlot1 = SessionData.PlayerInventory.RemoveItem(new ItemInstance(item1), item1Quantity);
            if (removedSlot1)
            {
                bool removedSlot2 = SessionData.PlayerInventory.RemoveItem(new ItemInstance(item2), item2Quantity);
                if (removedSlot2)
                {
#if UNITY_EDITOR
                    ...
                    else
                    {
                        ...
                        SessionData.PlayerInventory.AddItem(new ItemInstance(item2), item2Quantity);
#if UNITY_EDITOR
                ...
                SessionData.PlayerInventory.AddItem(new ItemInstance(item1), item1Quantity);
            }
        }
