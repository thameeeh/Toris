...
public static class BiomeFeatureBuilder
{
    ...
    {
        if (ctx.Profile.platformGroundTile == null) return;
        // staged platform (tune later)
        ctx.Stamps.StampRectGround(ctx.ActiveBiome.OriginTile, 9, 9, ctx.Profile.platformGroundTile);
    }
    ...
    private static void BuildRoadAndGate(WorldContext ctx, Dir dir)
    {
        ...
        // begin outside platform
        Vector2Int p = origin + step * 6;

        Vector2Int lastLand = p;
        int max = Mathf.Max(128, ctx.Profile.maxRoadScanTiles);
    ...
    {
        if (ctx.Profile.roadTile == null) return;

        // deterministic 3–5 width variation
        uint h = DeterministicHash.Hash((uint)ctx.ActiveBiome.Seed, center.x, center.y, 5001);
        float r = DeterministicHash.Hash01(h);

        int minW = Mathf.Clamp(ctx.Profile.roadWidthMin, 3, 5);
        int maxW = Mathf.Clamp(ctx.Profile.roadWidthMax, 3, 5);
        ...
        for (int i = -half; i <= half; i++)
            ctx.Stamps.SetGround(center + perp * i, ctx.Profile.roadTile);
    }

    private static void StampGate(WorldContext ctx, Vector2Int gateCenter)
    {
        int s = Mathf.Max(1, ctx.Profile.gateSize); // 7
        if (ctx.Profile.gateGroundTile != null)
            ctx.Stamps.StampRectGround(gateCenter, s, s, ctx.Profile.gateGroundTile);
